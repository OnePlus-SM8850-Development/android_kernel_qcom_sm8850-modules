			  ...
			  unsigned char *buf, unsigned int len)
{
	...
	ret = goodix_flash_cmd(cd, FLASH_CMD_RW_FINISH, FLASH_CMD_STATE_OKAY,
			       50);
	if (ret) {
		ts_err(dev, "failed read flash ready state");
		goto read_end;
	}
				      ...
				      struct goodix_ic_info_v2 *info_v2)
{
	...
	memcpy(info->parm.active_scan_rate, info_v2->param_pkg.active_scan_rate,
		info->parm.active_scan_rate_num * 2);
	info->parm.mutual_freq_num = info_v2->param_pkg.mutual_freq_num;
	memcpy(info->parm.mutual_freq, info_v2->param_pkg.mutual_freq,
		info->parm.mutual_freq_num * 2);
	info->parm.self_tx_freq_num = info_v2->param_pkg.self_tx_freq_num;
	memcpy(info->parm.self_tx_freq, info_v2->param_pkg.self_tx_freq,
		info->parm.self_tx_freq_num * 2);
	info->parm.self_rx_freq_num = info_v2->param_pkg.self_rx_freq_num;
	memcpy(info->parm.self_rx_freq, info_v2->param_pkg.self_rx_freq,
		info->parm.self_rx_freq_num * 2);
	info->parm.stylus_freq_num = info_v2->param_pkg.stylus_freq_num;
	memcpy(info->parm.stylus_freq, info_v2->param_pkg.stylus_freq,
		info->parm.stylus_freq_num * 2);
...
static int parse_ic_info_v2(struct goodix_ts_core *cd, const u8 *data)
{
	...
	while (offset < info_v2->length - 2) {
		...
		switch (pkg->pkg_tag) {
		case 0:
			memcpy((uint8_t *)ver_pkg + sizeof(*pkg), pkg->data,
				pkg->pkg_len - sizeof(*pkg));
			break;
		case 20:
			memcpy((uint8_t *)bsp_pkg + sizeof(*pkg), pkg->data,
				pkg->pkg_len - sizeof(*pkg));
			break;
		case 180:
			param_pkg->active_scan_rate_num = pkg->data[param_offset++];
			memcpy(param_pkg->active_scan_rate, pkg->data + param_offset,
				param_pkg->active_scan_rate_num * 2);
			param_offset += param_pkg->active_scan_rate_num * 2;
			param_pkg->mutual_freq_num = pkg->data[param_offset++];
			memcpy(param_pkg->mutual_freq, pkg->data + param_offset,
				param_pkg->mutual_freq_num * 2);
			param_offset += param_pkg->mutual_freq_num * 2;
			param_pkg->self_tx_freq_num = pkg->data[param_offset++];
			memcpy(param_pkg->self_tx_freq, pkg->data + param_offset,
				param_pkg->self_tx_freq_num * 2);
			param_offset += param_pkg->self_tx_freq_num * 2;
			param_pkg->self_rx_freq_num = pkg->data[param_offset++];
			memcpy(param_pkg->self_rx_freq, pkg->data + param_offset,
				param_pkg->self_rx_freq_num * 2);
			param_offset += param_pkg->self_rx_freq_num * 2;
			param_pkg->stylus_freq_num = pkg->data[param_offset++];
			memcpy(param_pkg->stylus_freq, pkg->data + param_offset,
				param_pkg->stylus_freq_num * 2);
			break;
		case 120:
			memcpy((uint8_t *)addr_pkg + sizeof(*pkg), pkg->data,
				pkg->pkg_len - sizeof(*pkg));
			break;
		case 160:
			memcpy((uint8_t *)protocol_pkg + sizeof(*pkg), pkg->data,
				pkg->pkg_len - sizeof(*pkg));
			break;
		case 230:
			memcpy((uint8_t *)extend_rep_pkg + sizeof(*pkg), pkg->data,
				pkg->pkg_len - sizeof(*pkg));
			break;
		case 300:
			memcpy((uint8_t *)extend_addr_pkg + sizeof(*pkg), pkg->data,
				pkg->pkg_len - sizeof(*pkg));
			break;
		case 290:
			memcpy((uint8_t *)scan_parm_pkg + sizeof(*pkg), pkg->data,
				pkg->pkg_len - sizeof(*pkg));
			break;
		default:
			break;
