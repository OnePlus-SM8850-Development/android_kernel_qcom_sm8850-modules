...
static int goodix_update_prepare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	if (cd->bus->sub_ic_type != IC_TYPE_SUB_MCU2) {
		...
		ts_info(dev, "enable misctl clock");
...
static int goodix_flash_firmware(struct fw_update_ctrl *fw_ctrl)
{
	...
	for (i = 1; i < fw_num && retry;) {
		...
		if (fw_x->size == 0) {
			ts_info(dev, "subsystem[%d], addr:0x%05X size is 0, skip",
				i, fw_x->flash_addr);
			i++;
			continue;
		}
			   ...
			   char *buf)
{
	...
	case UPSTA_PREPARING:
		snprintf(str, sizeof(str), "preparing");
		break;
	case UPSTA_UPDATING:
		snprintf(str, sizeof(str), "updating");
		break;
	case UPSTA_SUCCESS:
		snprintf(str, sizeof(str), "success");
		break;
	case UPSTA_FAILED:
	...
	case UPSTA_NOTWORK:
	default:
		snprintf(str, sizeof(str), "notwork");
		break;
	}

	r = scnprintf(buf, PAGE_SIZE, "result:%s  spend_time:%dms\n", str,
		     fw_ctrl->spend_time);
...
int goodix_fw_update_init(struct goodix_ts_core *cd)
{
	...
	if (cd->pdev->id == 0) {
		...
		snprintf(cd->update_ctrl.fw_name, sizeof(cd->update_ctrl.fw_name),
			"%s_%d.bin", prefix, cd->pdev->id);
	}
