			       ...
			       struct goodix_cfg_bin *cfg_bin)
{
	...
	cfg_bin->bin_data = kmemdup(firmware->data, cfg_bin->bin_data_len, GFP_KERNEL);
	if (!cfg_bin->bin_data) {
		ret = -ENOMEM;
		goto exit;
	}
...
refind_cfg:
	...
	for (i = 0; i < cfg_bin->head.pkg_num; i++) {
		...
		if (cfg_type >= GOODIX_MAX_CONFIG_GROUP) {
			ts_err(dev, "unsupported config type %d",
...
static int goodix_get_config_data(struct goodix_ts_core *cd, u8 sensor_id)
{
	...
	if (cd->pdev->id == 0) {
		...
		snprintf(cfg_name, sizeof(cfg_name), "%s_%d.bin", prefix, cd->pdev->id);
	}
