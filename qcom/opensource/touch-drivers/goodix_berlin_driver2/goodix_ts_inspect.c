...
static void proc_append_string(char *format_str, ...)
{
	...
	output_offset += vsnprintf(output_buf + output_offset,
		MAX_PROC_OUTPUT_SIZE - output_offset, format_str, args);
	va_end(args);
}
			...
			s32 rows, s32 col)
{
	...
	if (read_ret > 0) {
		ptr = buf;
		ptr = strnstr(ptr, target_name, size);
		if (!ptr) {
...
static int goodix_init_testlimits(struct goodix_ts_test *ts_test)
{
	...
	if (cd->pdev->id == 0) {
		snprintf(limit_file, sizeof(limit_file), "%s_%d.csv",
			GOODIX_TEST_FILE_NAME, cd->fw_version.sensor_id);
	} else {
		snprintf(limit_file, sizeof(limit_file), "%s_%d_%d.csv",
			GOODIX_TEST_FILE_NAME, cd->pdev->id,
...
static u32 map_die2pin(struct ts_test_params *test_params, u32 chn_num)
{
	...
	if (i >= max_drv_num)
		ts_err(NULL, "Failed found corresponding channel num:%d", chn_num);
	else
...
static void goodix_shortcircut_test(struct goodix_ts_test *ts_test)
{
	...
	if (ic_type == IC_TYPE_BERLIN_A) {
		...
		// test_param[1] = (ts_test->test_params.params_info->dft_short_threshold
		//		>> 8) & 0xFF;
		// test_param[2] =
		//		ts_test->test_params.params_info->short_diffcode_threshold & 0xFF;
		// test_param[3] = (ts_test->test_params.params_info->short_diffcode_threshold
		//		>> 8) & 0xFF;
		// test_param[4] = ts_test->test_params.params_info->short_test_dump_num & 0xFF;
		// test_param[5] = (ts_test->test_params.params_info->short_test_dump_num
		//		>> 8) & 0xFF;
...
static void goodix_version_test(struct goodix_ts_test *ts_test)
{
	...
	snprintf(ts_test->cur_version, sizeof(ts_test->cur_version),
...
static void goodix_reset_test(struct goodix_ts_test *ts_test)
{
	...
	if (memcmp(ver.patch_pid, "GEST", 4)) {
		ts_err(dev, "PID[%s] != GEST", ver.patch_pid);
		return;
	}
	ts_test_read_version(ts_test, &ver);
	if (memcmp(ver.patch_pid, cd->fw_version.patch_pid,
