					  ...
					  size_t count, loff_t *off)
{
	...
	char buf[64];
	char *sptr, *token;
	/* Value for the debug_cnss command. */
	char *cmd, *value;
	//val for other cmd
	u32 val;
...
void oplus_cnss_error_log_add(char *fmt, ...)
{
	...
	va_end(args);
	entry->time_s = oplus_conn_get_local_seconds();
	entry->next = NULL;
...
ssize_t icnss_show_cnss_debug(struct device_driver *driver, char *buf)
{
	...
	spin_lock_irqsave(&cel_lock, flags);
	if (!cel_head)
		length = scnprintf(buf, MAX_BUFFER_SIZE, "good");
	for (entry = cel_head; entry && length < MAX_BUFFER_SIZE - 1;
	     entry = entry->next)
		length += scnprintf(buf + length, MAX_BUFFER_SIZE - length,
